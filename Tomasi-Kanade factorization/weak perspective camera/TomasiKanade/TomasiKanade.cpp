...
using namespace cv;
using namespace std;
float* homogenus_solution(Mat Cfs) {
    ...
    float A = evecs.at<double>(5, 0);
    float B = evecs.at<double>(5, 1);
    float C = evecs.at<double>(5, 2);
    float D = evecs.at<double>(5, 3);
    float E = evecs.at<double>(5, 4);
    float F = evecs.at<double>(5, 5);
...
int main(int argc, char** argv)
{
    ...
    Mat coeffMtx(2 * NUMF, 6, CV_64F);
    Mat right(2 * NUMF, 1, CV_64F);
    ...
    for (int f = 0; f < NUMF; f++) {
        ...
        for (int i = 0; i < 6; i++) {
            if (i < 6){
            coeffMtx.at<double>(2 * f, i) = cf1.at<float>(0, i);
            coeffMtx.at<double>(2 * f + 1, i) = cf3.at<float>(0, i);
            }
            else {

                coeffMtx.at<double>(2 * f, 6) = 1;
                coeffMtx.at<double>(2 * f + 1, 6) = 1;

            }

        }

        right.at<double>(2 * f, 0) = 0;
        right.at<double>(2 * f + 1, 0) = 0;
        
    }

    cout << coeffMtx << "\n";
    float* res = homogenus_solution(coeffMtx);
    //Mat ls = ((coeffMtx.t() * coeffMtx).inv()) * coeffMtx.t() * right;
    Mat L(3, 3, CV_64F);

    L.at<double>(0, 0) = res[0];
    L.at<double>(0, 1) = L.at<double>(1, 0) = res[1];
    L.at<double>(0, 2) = L.at<double>(2, 0) = res[2];
    L.at<double>(1, 1) = res[3];
    L.at<double>(1, 2) = L.at<double>(2, 1) = res[4];
    L.at<double>(2, 2) = res[5];
    Mat U(3, 3, CV_64F);
    Mat S(3, 3, CV_64F);
    Mat V(3, 3, CV_64F);
