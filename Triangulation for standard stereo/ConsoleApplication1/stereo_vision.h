...
namespace stereo_vision
{
	...
	void geometry_triangulation(
		 cv::Point2d& u1,
		 cv::Point2d& u2,
		float f,
		float h,
		float b,
		float to_melimiter_covert,
		cv::Point3d& triongulated_point

	) {
		u1.x = u1.x / to_melimiter_covert;
		u1.y = u1.y / to_melimiter_covert;
		u2.x = u2.x / to_melimiter_covert;
		u2.y = u2.y / to_melimiter_covert;

		int ox = 404 / to_melimiter_covert;
		int oy = 309 / to_melimiter_covert;

		//float d = sqrt((u1.x - u2.x) * (u1.x - u2.x) + ((u1.y - u2.y) * (u1.y - u2.y)));
		float d = abs(u1.x - u2.x);

		cv::Point3d u1H(u1.x,u1.y,1);
		cv::Point3d u2H(u2.x,u2.y,1);
		
		
		//cv::Mat v1 = K.inv() * cv::Mat(u1H);
		float v1 = u1.y;
		float v2 = u2.y;
		cv::Mat RT;
		//cv::Mat v2 = T + RT * ((K.inv() * (cv::Mat(u2H))) - T);


		//float x =  (b * (u1.x + ox) / (d));
		//float y = (b * f * (u1.y - oy) / (u1.x + u2.x) );
		//float z = (b * f  / (d));

		float x = -1 * (b * (u1.x + u2.x) / (2 * d));

		//float x = - (b * (u1.x + u2.x)/2 * d);
		float y = b * v1 / d;
		float z = b * f / d;
		triongulated_point.x = x;
		triongulated_point.y = y;
		triongulated_point.z = z;
